...
#-------수정해야댐 ------------
#lex2.loc[행,열] 선택하면 integer형으로 숫자 띡 나오게 하고싶은데
#그러면 data를 어떻게 바꿔야 할까 그걸 고민하고 있슴
for k in lex2.index:
    print(lex2.loc[k,'Positive'])
...
#수정해야하는 부분때문에 여기는 아마 안돌아갈거임
#lex2.loc[k,'Positive']==1: 이 조건만 윗윗셀에서 말한 것 처럼 수정하면 됨!
#그거 빼고는 잘 돌아가는거 확인했음.
...
for i in range(1):
    ...
    for j in range(len(pre_text)): #형태소가 사전과 일치하면 count
        for k in lex2.index:
            if pre_text[j]==str(k) and lex2.loc[k,'Positive']==1:
                pos+=1
            if pre_text[j]==str(k) and lex2.loc[k,'Negative']==1:
                neg+=1
    if pos>neg:
